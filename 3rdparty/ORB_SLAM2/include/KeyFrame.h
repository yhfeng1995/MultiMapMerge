...
public:
    ...
    KeyFrame(LoadKeyFrame &F, Map* pMap, KeyFrameDatabase* pKFDB);
    ...
    void saveTemplet(std::fstream* fs) const;
...
protected:
    ...
    {
    public:
        LoadKeyFrame(ORBVocabulary* voc, Map* pMap);

        LoadKeyFrame(std::fstream* fs, ORBextractor* extractor, ORBVocabulary* voc, Map* pMap);

        LoadKeyFrame(const LoadKeyFrame & loadkf);

        void loadTemplet(std::fstream* fs);

        void load(std::fstream* fs, std::map<long unsigned int,  MapPoint*>& mnMapPoints);
    ...
    public:
        ...
        // Calibration parameters
        //! calculate from mk and from templet
